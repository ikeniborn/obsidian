...
generate_coturn_config() {
    ...
    # Detect Docker network subnet if ServerPeer is deployed
    local docker_subnet=""
    if docker ps --format '{{.Names}}' 2>/dev/null | grep -q "notes-serverpeer"; then
        docker_subnet=$(docker network inspect $(docker inspect notes-serverpeer-work --format='{{range $k, $v := .NetworkSettings.Networks}}{{$k}}{{end}}' 2>/dev/null) --format='{{range .IPAM.Config}}{{.Subnet}}{{end}}' 2>/dev/null || echo "")
        if [[ -n "$docker_subnet" ]]; then
            info "Detected Docker network: $docker_subnet - will allow P2P relay"
        fi
    fi
...
EOF
    ...
    # Allow Docker network if ServerPeer is deployed
    if [[ -n "$docker_subnet" ]]; then
        cat >> "$COTURN_CONFIG" << EOF

# Allow Docker network for ServerPeer P2P relay
allowed-peer-ip=${docker_subnet}
EOF
    fi

    cat >> "$COTURN_CONFIG" << EOF

# Deny other private IP ranges from being relayed
# NOTE: Do NOT use 0.0.0.0-255.255.255.255 (blocks everything)
denied-peer-ip=10.0.0.0-10.255.255.255
denied-peer-ip=192.168.0.0-192.168.255.255
# Only deny 172.16-31 ranges that are NOT our Docker network
denied-peer-ip=172.16.0.0-172.17.255.255
denied-peer-ip=172.19.0.0-172.31.255.255

EOF

    success "Coturn configuration generated at $COTURN_CONFIG"
    if [[ -n "$docker_subnet" ]]; then
        success "Docker network $docker_subnet is allowed for P2P relay"
    fi
}
