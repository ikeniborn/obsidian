...
**Obsidian Sync Server** - production-ready self-hosted CouchDB server for Obsidian note synchronization. This is an isolated application designed to integrate with Family Budget nginx or run standalone.

### Key Characteristics
- **Isolated deployment**: Separate docker-compose file, can run independently
- **Dual-mode**: Development (local) and Production (SSL with Let's Encrypt)
...
```
External nginx (from Family Budget)
└── Reverse proxy to CouchDB (port 5984)
    └── CouchDB container (familybudget-couchdb-notes)
        └── Data: /opt/notes/data

Docker Network: familybudget_familybudget (external)
Configuration: /opt/notes/.env
```

**Key Design Decisions:**
1. CouchDB port 5984 binds to `127.0.0.1` only - no external access
2. All HTTPS traffic goes through nginx reverse proxy
3. Uses external docker network to connect with Family Budget nginx
4. Deployment directory `/opt/notes` - separate from Family Budget `/opt/budget`
...
# Creates:
# - /opt/notes directory structure
# - /opt/notes/.env with dev credentials
# - Docker network familybudget_familybudget
```

### Running Locally
```bash
# Start Family Budget nginx (required dependency)
cd ~/familyBudget
...
# Access at: http://notes.localhost
...
### /opt/notes/.env
**Production:** Generated by `setup.sh`
...
**Development:** Generated by `dev-setup.sh`
- Fixed credentials: admin/dev_password_insecure
...
- Creates `/opt/notes` structure
- Generates dev .env with insecure credentials
...
- Run setup: `bash setup.sh` (production) or `bash dev-setup.sh` (development)
