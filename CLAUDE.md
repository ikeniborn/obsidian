...
### local.ini
CouchDB configuration (optimized 2026-01-11):
...
**Performance optimizations (Priority 1-3):**
- ✅ **Automatic compaction** - prevents database bloat
  - `check_interval = 300` - check every 5 minutes
  - `min_file_size = 52428800` - compact only databases >50MB
  - `_default = [{db_fragmentation, "20%"}]` - trigger at 20% fragmentation
- ✅ **Write optimization** - 3-5x faster bulk writes
...
- `max_dbs_open = 1000` - better multi-database performance
- `attachment_compression_level = 6` - balanced compression
- `os_process_timeout = 10000` - 10s timeout for complex views

**Monitoring (Priority 7):**
- `compression = true` - HTTP gzip compression
- `level = warning` - reduced log noise
- Stats collection every 10 seconds

See `docs/couchdb-optimization-recommendations.md` for full analysis
...
**monitor-couchdb.sh** (NEW: 2026-01-11)
- Health monitoring for CouchDB databases
- Checks fragmentation levels (warns at >20%, alerts at >30%)
- Monitors disk space usage
- Verifies compaction daemon configuration
- Reports active compactions
- Displays Docker container resource usage
- Logs to `/opt/notes/logs/health.log`
...
### Regular Checks
```bash
# NEW (2026-01-11): Comprehensive health monitoring
bash /opt/notes/scripts/monitor-couchdb.sh
# Checks: fragmentation, disk space, compaction daemon, active tasks, resource usage
...
- **Health monitoring** (NEW): `/opt/notes/logs/health.log`
