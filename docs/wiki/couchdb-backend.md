...
- **Automatic compaction (smoosh, CouchDB 3.x) — DISABLED 2026-06-18**: `[smoosh]` now ships with **empty** `db_channels`/`view_channels`, so the daemon stays idle. The channel definitions remain (`[smoosh.ratio_dbs]` file/active ratio > `1.25` ~20% fragmentation; `[smoosh.slack_dbs]` wasted space > `209715200` 200MB; `wait = 30`) but are not wired in. Disabled after a disk-full crash loop: on a near-full disk smoosh writes a temporary `*.couch.compact.data` the size of the active db (~17G for `work`), the full disk kills it mid-way with `{error,enospc}`, and it retries every cycle — leaving orphan `*.compact.*` files and returning HTTP 500 on write. Re-enable (restore the channel lists) only after DB bloat is reset and the disk has >2x the largest db free. (Note: the 2.x `[compaction_daemon]`/`[compactions]` syntax is ignored in 3.x.) See [[troubleshooting#HTTP 500 — disk full / compaction loop]].
